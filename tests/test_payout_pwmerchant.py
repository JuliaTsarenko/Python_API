...
class TestPayoutPayway:
    ...
    #                   payway_id=admin.payway['visamc']['id'], is_active=True, is_bound=True)
    ...
    #     # print('lid', user1.pwmerchant_KUNA.lid)
    #     # pprint.pprint(user1.resp_delegate)
    #     assert user1.resp_delegate['account_amount'] == '0.01'
    #     assert user1.resp_delegate['in_amount'] == '0.01'
    #     assert user1.resp_delegate['in_fee_amount'] == '0'
    ...
    def test_payout_3_1(self): # Перевод суммы равной сумме на счету списания
        ...
        user1.pwmerchant_PAYEER.payout_create(payway='payeer', amount='1.02', out_curr='RUB', payee='P14812343')
        # pprint.pprint(user1.pwmerchant_PAYEER.resp_payout_create)
        # time.sleep(2)
    ...
    def test_payout_4_1(self): # Вывод суммы равной техническому максимуму по таблице pwcurrency
        ...
        admin.set_wallet_amount(balance=bl(0.99999), currency='BTC', merch_lid=user1.pwmerchant_BTC.lid)
        admin.set_fee(mult=0, add=0, _min=0, _max=0, around='ceil', tp=10, currency_id=admin.currency['BTC'],
                      payway_id=admin.payway['btc']['id'], is_active=False, merchant_id=user1.pwmerchant_BTC.id)
        admin.set_fee(mult=0, add=0, _min=0, _max=0, around='ceil', tp=10, currency_id=admin.currency['BTC'],
                      payway_id=admin.payway['btc']['id'], is_active=False)
        admin.set_pwmerchactive(merch_id=user1.pwmerchant_BTC.id, payway_id=admin.payway['btc']['id'], is_active=True)
        ...
        user1.pwmerchant_BTC.payout_create(payway='btc', amount='0.99999', out_curr='BTC', payee='32LdQGCG1PHYNP2sRkZgrP6UAfQYTSkshx')
        # pprint.pprint(user1.merchant1.resp_payout_create)
        # assert user1.merchant1.resp_payout_create['result']['status'] == 'done'
        assert user1.pwmerchant_BTC.resp_payout_create['result']['in_amount'] == '0.99999'
        assert user1.pwmerchant_BTC.resp_payout_create['result']['out_amount'] == '0.99999'
        assert user1.pwmerchant_BTC.resp_payout_create['result']['in_fee_amount'] == '0'
        assert user1.pwmerchant_BTC.resp_payout_create['result']['account_amount'] == '0.99999'
        assert user1.pwmerchant_BTC.resp_payout_create['result']['out_fee_amount'] == '0'
    ...
    def test_payout_5_1(self):
        ...
        admin.set_wallet_amount(balance=bl(11.5), currency='UAH', merch_lid=user1.pwmerchant_PRIVAT24.lid)
        admin.set_fee(mult=0, add=0, _min=0, _max=0, around='ceil', tp=10, currency_id=admin.currency['UAH'],
                      payway_id=admin.payway['privat24']['id'], is_active=True, merchant_id=user1.pwmerchant_PRIVAT24.id)
        ...
        admin.set_pwmerchactive(merch_id=user1.pwmerchant_PRIVAT24.id, payway_id=admin.payway['privat24']['id'], is_active=True)
        ...
        user1.delegate(params={'merch_model': 'payout', 'merch_method': 'create', 'externalid': user1.ex_id(),
                               ...
                               'm_lid': str(user1.pwmerchant_PRIVAT24.lid), 'payee': '4731185613244273'})
        # pprint.pprint(user1.resp_delegate)
        # assert user1.resp_delegate['status'] == 'done'
        ...
        assert user1.pwmerchant_PRIVAT24.balance(curr='UAH') == '0'

    def test_payout_5_3(self):
        """ Payout to privat24 10 UAH: UAH to UAN by OWNER with common percent fee 15% for payout. """
        admin.set_merchant(lid=user1.pwmerchant_KUNA.lid, is_customfee=True)
        admin.set_wallet_amount(balance=bl(15), currency='UAH', merch_lid=user1.pwmerchant_KUNA.lid)
        admin.set_fee(mult=0, add=0, _min=0, _max=0, around='ceil', tp=10, currency_id=admin.currency['UAH'],
                      payway_id=admin.payway['privat24']['id'], is_active=False, merchant_id=user1.pwmerchant_PRIVAT24.id)
        admin.set_fee(mult=pers(15), add=0, _min=0, _max=0, around='ceil', tp=10, currency_id=admin.currency['UAH'],
                      payway_id=admin.payway['privat24']['id'], is_active=True)
        admin.set_fee(mult=0, add=0, _min=0, _max=0, around='ceil', tp=0, currency_id=admin.currency['UAH'],
                      payway_id=admin.payway['kuna']['id'], is_active=False, merchant_id=user1.pwmerchant_KUNA.id)
        admin.set_fee(mult=pers(15), add=0, _min=0, _max=0, around='ceil', tp=0, currency_id=admin.currency['UAH'],
                      payway_id=admin.payway['kuna']['id'], is_active=True)
        admin.set_pwmerchactive(merch_id=user1.pwmerchant_KUNA.id, payway_id=admin.payway['privat24']['id'], is_active=True)
        admin.set_pwcurrency_min_max(payway=admin.payway['privat24']['id'], is_out=True, currency='UAH', is_active=True,
                                     tech_min=bl(1), tech_max=bl(100))
        admin.set_pwcurrency_min_max(payway=admin.payway['kuna']['id'], is_out=True, currency='UAH', is_active=True,
                                     tech_min=bl(1), tech_max=bl(100))
        user1.delegate(params={'merch_model': 'payout', 'merch_method': 'create', 'externalid': user1.ex_id(),
                               ...
                               'm_lid': str(user1.pwmerchant_KUNA.lid), 'payee': '4731185613244273'})
        pprint.pprint(user1.resp_delegate)
        assert user1.resp_delegate['account_amount'] == '13.23'
        assert user1.resp_delegate['in_amount'] == '10'
        assert user1.resp_delegate['in_fee_amount'] == '1.73'
        assert user1.resp_delegate['out_amount'] == '10'
        assert user1.resp_delegate['out_fee_amount'] == '1.5'
        assert user1.resp_delegate['reqdata']['amount'] == '10'
        assert user1.pwmerchant_KUNA.balance(curr='UAH') == '1.77'

    def test_payout_6_1(self):
        """ Payout to ltc 0.5 LTC: LTC to LTC by MERCHANT with common absolute fee 0.001 LTC for payout. """
        admin.set_wallet_amount(balance=bl(1), currency='LTC', merch_lid=user1.pwmerchant_LTC.lid)
        admin.set_fee(mult=0, add=0, _min=0, _max=0, around='ceil', tp=10, currency_id=admin.currency['LTC'],
                      payway_id=admin.payway['ltc']['id'], is_active=False, merchant_id=user1.pwmerchant_LTC.id,
                      is_bound=True)
        admin.set_fee(mult=0, add=bl(0.001), _min=0, _max=0, around='ceil', tp=10, currency_id=admin.currency['LTC'],
                      payway_id=admin.payway['ltc']['id'], is_active=True, is_bound=True)
        admin.set_pwmerchactive(merch_id=user1.pwmerchant_LTC.id, payway_id=admin.payway['ltc']['id'], is_active=True)
        ...
        user1.pwmerchant_LTC.payout_create(payway='ltc', amount='0.5', out_curr='LTC',
                                           payee='M93SfGQPnNp3dEdPJx1GrizvNWUFA1hSVi')
        # pprint.pprint(user1.pwmerchant_LTC.resp_payout_create)
        assert user1.pwmerchant_LTC.resp_payout_create['result']['in_amount'] == '0.5'
        assert user1.pwmerchant_LTC.resp_payout_create['result']['out_amount'] == '0.5'
        assert user1.pwmerchant_LTC.resp_payout_create['result']['in_fee_amount'] == '0.001'
        assert user1.pwmerchant_LTC.resp_payout_create['result']['account_amount'] == '0.501'
        assert user1.pwmerchant_LTC.resp_payout_create['result']['out_fee_amount'] == '0.001'
        # assert user1.merchant1.balance(curr='LTC') == '0.499'

    @pytest.mark.skip
    def test_payout_7_1(self):
        """ Payout to qiwi 10 RUB: RUB to RUB by OWNER with common percent fee 5.5% for payout
         and common absolute fee 1 RUB for payout."""
        admin.set_wallet_amount(balance=bl(15), currency='RUB', merch_lid=user1.pwmerchant_QIWI.lid)
        admin.set_fee(mult=0, add=0, _min=0, _max=0, around='ceil', tp=10, currency_id=admin.currency['RUB'],
                      payway_id=admin.payway['qiwi']['id'], is_active=True, merchant_id=user1.pwmerchant_QIWI.id)
        admin.set_fee(mult=bl(0.055), add=bl(1), _min=0, _max=0, around='ceil', tp=10, currency_id=admin.currency['RUB'],
                      payway_id=admin.payway['qiwi']['id'], is_active=True, is_bound=True)
        admin.set_pwmerchactive(merch_id=user1.pwmerchant_QIWI.id, payway_id=admin.payway['qiwi']['id'], is_active=True)
        ...
        user1.delegate(params={'merch_model': 'payout', 'merch_method': 'create', 'externalid': user1.ex_id(),
                               ...
                               'm_lid': str(user1.pwmerchant_QIWI.lid), 'payee': '380965781066'})
        # pprint.pprint(user1.resp_delegate)
        ...
        assert user1.resp_delegate['reqdata']['amount'] == '10'
        assert user1.pwmerchant_QIWI.balance(curr='RUB') == '3.45'

    @pytest.mark.skip
    def test_payout_7_3(self):
        """ Payout to qiwi 10 RUB: RUB to RUB by OWNER with common percent fee 5.5% for payout
         and common absolute fee 1 RUB for payout."""
        admin.set_merchant(lid=user1.pwmerchant_VISAMC.lid, is_customfee=True)
        admin.set_wallet_amount(balance=bl(15), currency='RUB', merch_lid=user1.pwmerchant_VISAMC.lid)
        admin.set_fee(mult=0, add=0, _min=0, _max=0, around='ceil', tp=10, currency_id=admin.currency['RUB'],
                      payway_id=admin.payway['qiwi']['id'], is_active=True, merchant_id=user1.pwmerchant_VISAMC.id)
        admin.set_fee(mult=bl(0.055), add=bl(1), _min=0, _max=0, around='ceil', tp=10, currency_id=admin.currency['RUB'],
                      payway_id=admin.payway['qiwi']['id'], is_active=True)
        admin.set_fee(mult=0, add=0, _min=0, _max=0, around='ceil', tp=0, currency_id=admin.currency['RUB'],
                      payway_id=admin.payway['visamc']['id'], is_active=True, merchant_id=user1.pwmerchant_VISAMC.id)
        admin.set_fee(mult=bl(0.055), add=bl(1), _min=0, _max=0, around='ceil', tp=0, currency_id=admin.currency['RUB'],
                      payway_id=admin.payway['visamc']['id'], is_active=True)
        admin.set_pwmerchactive(merch_id=user1.pwmerchant_VISAMC.id, payway_id=admin.payway['qiwi']['id'], is_active=True)
        admin.set_pwcurrency_min_max(payway=admin.payway['qiwi']['id'], is_out=True, currency='RUB', is_active=True,
                                     tech_min=bl(1), tech_max=bl(100))
        user1.delegate(params={'merch_model': 'payout', 'merch_method': 'create', 'externalid': user1.ex_id(),
                               'payway': 'qiwi', 'amount': '10', 'out_curr': 'RUB',
                               'm_lid': str(user1.pwmerchant_VISAMC.lid), 'payee': '380965781066'})
        # pprint.pprint(user1.resp_delegate)
        assert user1.resp_delegate['account_amount'] == '13.1'
        assert user1.resp_delegate['in_amount'] == '10'
        assert user1.resp_delegate['in_fee_amount'] == '3.1'
        assert user1.resp_delegate['out_amount'] == '10'
        assert user1.resp_delegate['out_fee_amount'] == '1.55'
        assert user1.resp_delegate['reqdata']['amount'] == '10'
        assert user1.pwmerchant_VISAMC.balance(curr='RUB') == '1.9'

    def test_payout_8_3(self):
        ...
        admin.set_merchant(lid=user1.pwmerchant_EXMO.lid, is_customfee=True)
        admin.set_wallet_amount(balance=bl(15), currency='USD', merch_lid=user1.pwmerchant_EXMO.lid)
        admin.set_fee(mult=pers(5.5), add=bl(1), _min=0, _max=0, around='ceil', tp=10,
                      currency_id=admin.currency['USD'], payway_id=admin.payway['perfect']['id'],
                      is_active=True, merchant_id=user1.pwmerchant_EXMO.id)
        ...
        admin.set_fee(mult=pers(5.5), add=bl(1), _min=0, _max=0, around='ceil', tp=0,
        ...
        admin.set_fee(mult=0, add=0, _min=0, _max=0, around='ceil', tp=0, currency_id=admin.currency['USD'],
                      payway_id=admin.payway['exmo']['id'], is_active=True)
        admin.set_pwmerchactive(merch_id=user1.pwmerchant_EXMO.id, payway_id=admin.payway['perfect']['id'],
                                is_active=True)
        ...
        admin.set_pwcurrency_min_max(payway=admin.payway['exmo']['id'], is_out=True, currency='USD',
                                     is_active=True, tech_min=bl(0.01), tech_max=bl(15))
        user1.pwmerchant_EXMO.payout_create(payway='perfect', amount='10', out_curr='USD', payee='U6768929')
        # print('lid', user1.pwmerchant_EXMO.lid)
        # pprint.pprint(user1.pwmerchant_EXMO.resp_payout_create)
        assert user1.pwmerchant_EXMO.resp_payout_create['result']['account_amount'] == '13.1'
        assert user1.pwmerchant_EXMO.resp_payout_create['result']['in_amount'] == '10'
        assert user1.pwmerchant_EXMO.resp_payout_create['result']['out_amount'] == '10'
        assert user1.pwmerchant_EXMO.resp_payout_create['result']['in_fee_amount'] == '3.1'
        assert user1.pwmerchant_EXMO.resp_payout_create['result']['out_fee_amount'] == '1.55'
        assert user1.pwmerchant_EXMO.resp_payout_create['result']['reqdata']['amount'] == '10'
        assert user1.pwmerchant_EXMO.balance(curr='USD') == '1.9'
        admin.set_merchant(lid=user1.pwmerchant_EXMO.lid)

    def test_payout_9_1(self):
        ...
        and with personal absolute fee 1 USD  payout."""
        admin.set_merchant(lid=user1.pwmerchant_CASH_KIEV.lid, is_customfee=True)
        admin.set_wallet_amount(balance=bl(15), currency='USD', merch_lid=user1.pwmerchant_CASH_KIEV.lid)
        admin.set_fee(mult=bl(0.055), add=bl(1), _min=0, _max=0, around='ceil', tp=10,
                      ...
                      is_active=True, merchant_id=user1.pwmerchant_CASH_KIEV.id, is_bound=True)
        ...
        admin.set_pwmerchactive(merch_id=user1.pwmerchant_CASH_KIEV.id, payway_id=admin.payway['cash_kiev']['id'],
                                is_active=True)
        ...
        user1.delegate(params={'merch_model': 'payout', 'merch_method': 'create', 'externalid': user1.ex_id(),
                               ...
                               'm_lid': str(user1.pwmerchant_CASH_KIEV.lid), 'contact': '380965781066'})
        # print('lid', user1.pwmerchant_CASH_KIEV.lid)
        # pprint.pprint(user1.resp_delegate)
        assert user1.resp_delegate['account_amount'] == '13'
        assert user1.resp_delegate['in_amount'] == '10'
        assert user1.resp_delegate['in_fee_amount'] == '3'
        assert user1.resp_delegate['out_amount'] == '10'
        assert user1.resp_delegate['out_fee_amount'] == '3'
        assert user1.resp_delegate['reqdata']['amount'] == '10'
        assert user1.pwmerchant_CASH_KIEV.balance(curr='USD') == '2'
        admin.set_merchant(lid=user1.pwmerchant_CASH_KIEV.lid)

    def test_payout_9_3(self):
        """ Payout to cash_kiev  10 USD: USD to USD by OWNER with common percent fee 10% for payout
        and with common absolute fee 2 USD for payout with personal percent fee 5.5% for payout
        and with personal absolute fee 1 USD  payout."""
        admin.set_merchant(lid=user1.pwmerchant_PERFECT.lid, is_customfee=True)
        admin.set_wallet_amount(balance=bl(16), currency='USD', merch_lid=user1.pwmerchant_PERFECT.lid)
        admin.set_fee(mult=bl(0.055), add=bl(1), _min=0, _max=0, around='ceil', tp=10,
                      currency_id=admin.currency['USD'], payway_id=admin.payway['cash_kiev']['id'],
                      is_active=True, merchant_id=user1.pwmerchant_PERFECT.id)
        admin.set_fee(mult=bl(0.1), add=bl(2), _min=0, _max=0, around='ceil', tp=10,
                      currency_id=admin.currency['USD'], payway_id=admin.payway['cash_kiev']['id'], is_active=True)
        admin.set_fee(mult=bl(0.055), add=bl(1), _min=0, _max=0, around='ceil', tp=0,
                      currency_id=admin.currency['USD'], payway_id=admin.payway['perfect']['id'],
                      is_active=True, merchant_id=user1.pwmerchant_PERFECT.id)
        admin.set_fee(mult=bl(0.1), add=bl(2), _min=0, _max=0, around='ceil', tp=0,
                      currency_id=admin.currency['USD'], payway_id=admin.payway['perfect']['id'], is_active=True)
        admin.set_payways(name='cash_kiev')
        admin.set_pwmerchactive(merch_id=user1.pwmerchant_PERFECT.id, payway_id=admin.payway['cash_kiev']['id'],
                                is_active=True)
        admin.set_pwcurrency_min_max(payway=admin.payway['cash_kiev']['id'], is_out=True, currency='USD',
                                     is_active=True, tech_min=bl(1), tech_max=bl(15))
        admin.set_pwcurrency_min_max(payway=admin.payway['perfect']['id'], is_out=True, currency='USD',
        ...
        user1.delegate(params={'merch_model': 'payout', 'merch_method': 'create', 'externalid': user1.ex_id(),
                               'payway': 'cash_kiev', 'amount': '10', 'out_curr': 'USD',
                               'm_lid': str(user1.pwmerchant_PERFECT.lid), 'contact': '380965781066'})
        # print('lid', user1.pwmerchant_PERFECT.lid)
        # pprint.pprint(user1.resp_delegate)
        assert user1.resp_delegate['account_amount'] == '13.1'
        assert user1.resp_delegate['in_amount'] == '10'
        assert user1.resp_delegate['in_fee_amount'] == '3.1'
        ...
        assert user1.resp_delegate['out_fee_amount'] == '1.55'
        assert user1.resp_delegate['reqdata']['amount'] == '10'
        assert user1.pwmerchant_PERFECT.balance(curr='USD') == '2.9'
        admin.set_merchant(lid=user1.pwmerchant_PERFECT.lid)

    def test_payout_10_2(self):
        """ Payout to visamc 50 RUB: UAH to RUB by OWNER with internal exchange
        and without fee for exchange. """
        admin.set_wallet_amount(balance=bl(55), currency='UAH', merch_lid=user1.pwmerchant_VISAMC.lid)
        ...
        admin.set_fee(mult=0, add=0, _min=0, _max=0, around='ceil', tp=30, currency_id=admin.currency['UAH'],
                      payway_id=admin.payway['visamc']['id'], is_active=True, is_bound=True)
        admin.set_fee(mult=0, add=0, _min=0, _max=0, around='ceil', tp=30, currency_id=admin.currency['RUB'],
                      payway_id=admin.payway['visamc']['id'], is_active=True, merchant_id=user1.pwmerchant_VISAMC.id,
                      is_bound=True)
        admin.set_fee(mult=0, add=0, _min=0, _max=0, around='ceil', tp=30, currency_id=admin.currency['RUB'],
                      payway_id=admin.payway['visamc']['id'], is_active=True, is_bound=True)
        admin.set_pwmerchactive(merch_id=user1.pwmerchant_VISAMC.id, payway_id=admin.payway['paymer']['id'],
                                is_active=True)
        ...
        admin.set_pwcurrency_min_max(payway=admin.payway['visamc']['id'], is_out=True, currency='RUB',
                                     is_active=True, tech_min=bl(0.01), tech_max=bl(50))
        user1.delegate(params={'merch_model': 'payout', 'merch_method': 'create', 'externalid': user1.ex_id(),
                               'payway': 'visamc', 'amount': '50', 'out_curr': 'RUB', 'in_curr': 'UAH',
                               'm_lid': str(user1.pwmerchant_VISAMC.lid), 'payee': '4731185613244273'})
        pprint.pprint(user1.resp_delegate)
        ...
        assert user1.pwmerchant_VISAMC.balance(curr='UAH') == '1.24'

    def test_payout_10_4(self):
