...
class Admin:
    ...
    def autorization(self, email, pwd):
        ...
        r = requests.post(url=self.user_url,
                          ...
                          verify=False)
        return {'x-token': loads(r.text)['result']['session']['token']}
    ...
    def get_order(self, merchant_id, count=20, id=None):
        json = {'model': 'order', 'method': 'select', 'rng': [0, count],
                            'key': {'merchant_id': merchant_id}}
        if id: json['key']['id'] = id
        r = requests.post(url=self.admin_url,
                      json=json,
                      headers=self.headers, verify=False)
        order_list = []
        order_list['count']=loads(r.text)[0]['count']
        fields = loads(r.text)[0]['fields']
        for i in loads(r.text)[0]['data']:
        ...
        if loads(r.text)[0]['data']:
            ...
            return 'No order(s) for this query.'
...
if __name__ == '__main__':
    ...
    admin.get_crypto_adress(_filter='merchant_id', value=703687441778420)
    ...
    # admin = Admin(token='C_0E6Z4dLc8MudkSBCVx_p67E0vZU5F6e8aJcxqPt__Rd8oN7_1mgfRxBuNoa6xdKlgi'
    # print(admin.front_params)
